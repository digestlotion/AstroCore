...
public class SteamMinerMachine extends WorkableMultiblockMachine implements IMiner, IDisplayUIMachine {

    protected static final ManagedFieldHolder MANAGED_FIELD_HOLDER = new ManagedFieldHolder(
            SteamMinerMachine.class, WorkableMultiblockMachine.MANAGED_FIELD_HOLDER);

    public static final int STEAM_PER_PARALLEL = 16;
    public static final int WATER_PER_PARALLEL = 6;
    public static final int MAX_PARALLELS = 8;
    public static final int BASE_SPEED = 240;
    ...
    public SteamMinerMachine(IMachineBlockEntity holder, Object... args) {
        super(holder);
    ...
    public boolean drainInput(boolean simulate) {
        ...
        int steamNeeded = STEAM_PER_PARALLEL * parallels;
        int fluidNeeded = WATER_PER_PARALLEL * parallels;

        NotifiableFluidTank steamTank = findSteamTank();
        if (steamTank == null) return false;
        if (steamTank.getFluidInTank(0).getAmount() < steamNeeded) return false;
        ...
        if (!simulate) {
            steamTank.drainInternal(new FluidStack(GTMaterials.Steam.getFluid(), steamNeeded),
                    IFluidHandlerModifiable.FluidAction.EXECUTE);
            if (hasDrilling) {
                NotifiableFluidTank tank = findDrillingFluidTank();
                if (tank != null) tank.drainInternal(
                        new FluidStack(GTMaterials.DrillingFluid.getFluid(), fluidNeeded),
                        IFluidHandlerModifiable.FluidAction.EXECUTE);
            } else {
                NotifiableFluidTank tank = findWaterTank();
                if (tank != null) tank.drainInternal(
                        new FluidStack(GTMaterials.Water.getFluid(), fluidNeeded),
                        IFluidHandlerModifiable.FluidAction.EXECUTE);
            }
        }
        return true;
            ...
            for (var hl : part.getRecipeHandlers()) {
                ...
                for (var h : hl.getCapability(FluidRecipeCapability.CAP)) {
                    if (h instanceof NotifiableFluidTank nft &&
                            nft.isFluidValid(0, GTMaterials.Steam.getFluid(1)))
                        return nft;
                }
            }
        }
    ...
    private void minerTick() {
        ...
        if (!getRecipeLogic().isWorking()) {
            activeParallels = clampParallel(targetParallel);
        }
    ...
    public void addDisplayText(List<Component> textList) {
        ...
        if (conflict) {
            ...
            textList.add(Component.translatable("gtceu.multiblock.large_miner.done")
                    .withStyle(ChatFormatting.GREEN));
        } else if (findSteamTank() == null ||
                findSteamTank().getFluidInTank(0).getAmount() < STEAM_PER_PARALLEL * parallels) {
